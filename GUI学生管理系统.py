# -*- coding: utf-8 -*-
# @Time    : 2023-03-10 20:57
# @Author  : 邓波
# @FileName: GUI学生管理系统.py
# @Software: PyCharm
import tkinter
...
# 添加函数
def addInfo():
    if entryId.get() and entryName.get() and entrySex.get() and entryPython.get() and entrydatabase.get() and entryClanguage.get():
        x = dataTreeview.get_children()
        for item in x:
            dataTreeview.delete(item)
        values = (entryId.get(), entryName.get(), entrySex.get(), entryPython.get(), entrydatabase.get(), entryClanguage.get())
        con = sqlite3.connect(database)
        cur = con.cursor()
...
# 添加组件
tkinter.Button(root,text='添加',width=40,command=addInfo).place(x=20, y=210, width=40, height=20)
tkinter.Button(root,text='删除已选',width=100,command=deleteSelection).place(x=80, y=210, width=100, height=20)
tkinter.Button(root,text='查询',width=40,command=showAllInfo).place(x=200, y=210, width=40, height=20)

# 数据列表显示模块
dataTreeview = ttk.Treeview(root, show='headings', column=('id','name', 'sex', 'python_score', 'database_score', 'c_language_score'))
